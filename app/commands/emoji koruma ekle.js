...
.addField(`**Nasıl Açacağım?**`,"`v!eklenen-emoji-koruma aç`\n")
.addField(`**Nasıl Kapatacağım?**`,"`v!eklenne-emoji-koruma kapat`")

if(!args[0]) return message.channel.send(emb)

const invex11 = new Discord.MessageEmbed()
.setColor('BLUE')
  .setDescription(`<a:yes:793101776651681813> Eklenen Emoji Koruma sistemi başarıyla açıldı.`)
...
.setColor('BLUE')
.setDescription(`<a:yes:793101776651681813> Eklenen Emoji Koruma sistemi kapatıldı.`)
  .setAuthor(`${message.author.tag} tarafından istendi.`, message.author.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
    .setFooter(client.user.username,client.user.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
      .setTimestamp()
...
.setColor('BLUE')
.setDescription(`<a:no:793101866673111040> Eklenen Emoji Koruma sistemi zaten kapalı durumda.`)
  .setAuthor(`${message.author.tag} tarafından istendi.`, message.author.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
    .setFooter(client.user.username,client.user.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
      .setTimestamp()
...
.setColor('BLUE')
.setDescription(`<a:no:793101866673111040> Eklenen Emoji Koruma sistemi zaten açık durumda.`)
  .setAuthor(`${message.author.tag} tarafından istendi.`, message.author.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
    .setFooter(client.user.username,client.user.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
      .setTimestamp()
if(args[0] === "aç"){
...
exports.conf = {
  enabled: true,
  guildOnly: true,
  aliases: ["eklenen-emoji-koruma"],
  permLevel: 0
}

exports.help = {
  name: 'eklenen-emoji-koruma'
};