...
.setColor('BLUE')
  .setDescription(`**<a:no:793101866673111040> Bu işlemi yapabilmek için \`v!yetkili\` role sahip olmalı veya \`Sunucu sahibi\` olmalısınız.**`)
...
.setDescription(`**Düzenlenen Kanal Koruma Sistem Bilgilendirmesi**\n *Düzenlenen kanalları eski haline getirir.*\n`)
.addField(`**Nasıl Açacağım?**`,"`v!düzenlenen-kanal-koruma aç`\n")
.addField(`**Nasıl Kapatacağım?**`,"`v!düzenlenen-kanal-koruma kapat`")
...
.setColor('BLUE')
.setDescription(`<a:no:793101866673111040> Düzenlenen Kanal Koruma sistemi zaten kapalı durumda.`)
  .setAuthor(`${message.author.tag} tarafından istendi.`, message.author.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
    .setFooter(client.user.username,client.user.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
      .setTimestamp()
...
.setColor('BLUE')
.setDescription(`<a:no:793101866673111040> Düzenlenen Kanal Koruma sistemi zaten açık durumda.`)
  .setAuthor(`${message.author.tag} tarafından istendi.`, message.author.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
    .setFooter(client.user.username,client.user.avatarURL({dynamic: true,format: "gif",format: "png",format: "jpg",size: 2048}))
      .setTimestamp()
