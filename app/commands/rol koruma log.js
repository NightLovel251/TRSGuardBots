...
.setDescription(`**Rol-Log Sistem Bilgilendirmesi***\n *Rol Koruma Sistemi loglarını belirlediğiniz kanala gönderir.*\n`)
.addField(`**Nasıl Ayarlayacağım?**`,"`v!rol-log #kanal`\n")
.addField(`**Nasıl Kapatacağım?**`,"`v!rol-log kapat`")



if(!args[0]) return message.channel.send(emb)
let rollog = message.mentions.channels.first() || message.guild.channels.cache.find(r => r.id == args[0])
db.set(`rollog_${message.guild.id}`, `${rollog}`)

const invex11 = new Discord.MessageEmbed()
.setColor('BLUE')
  .setDescription(`Rol-Log kanalı başarıyla ${rollog} olarak ayarlandı.`)
...
db.set(`rollog.${message.guild.id}`, rollog.id)
